...
> Mise à jour M161 — cette décision a été resserrée après revue produit : **tout
> choix de lecture reste dans `/bibliotheque`**, sous forme de sélecteur inline
> adressable. Les anciennes routes de sélection `/parole`,
> `/parole/historique` et `/parole/semaines/:id` ne servent plus que de
> compatibilité et redirigent vers cet état. `/parole/paroisse/:ward_code`
> redirige vers le Cercle autorisé du profil courant ; seul
> `/parole/parcours/:id` subsiste comme parcours de Quiz distinct. Les passages
> historiques ci-dessous qui évoquent une vue détaillée de semaine ou une rama
> dans la Bibliothèque sont donc supersédés par cette règle.
...
- Recherche canonique : `Scriptures::QueryResolver`, route GET `/bibliotheque/recherche`, résolution exacte sans JavaScript et combobox Stimulus avec délai de 180 ms et annulation via `AbortController`.
- Navigation : dock, navigation desktop et hamburger utilisent `/bibliotheque`; profil, rama, semaine et sortie de parcours reviennent aux ancres stables de la Bibliothèque.
- Migration : `/parole` → `/bibliotheque`, `/parole/historique` → `#mes-ecritures`, `/parole/paroisse/:ward_code` → `#ma-rama`. Le code historique de rama n’est jamais consulté.
- Artwork : réutilisation de l’illustration Psaumes Refuge déjà approuvée et de ses renditions responsives manifestées ; aucun asset généré redondant.
- Preuves visuelles inspectées : 390×844, 768×1024 et 1440×900 dans `tmp/street-shots/scripture-library/`. Premier CTA visible, colonne unique, aucun dashboard, console sans erreur sévère.
- Tests verts : résolveur quatre langues et alias, chapitre/verset/plage/bornes, contrôleur recherche exacte/ambiguë/invalide, redirections, i18n, dock/hamburger, routes profondes, lecteur et Cercle — 71 tests / 3 087 assertions. Zeitwerk vert.
- Système visuel et interaction : 3 tests / 30 assertions — trois viewports, combobox clavier, focus d’ancre, mouvement réduit et console sans erreur sévère.
- Code supprimé : contrôleurs et vues des agrégateurs `StudyPrograms`, `StudyHistories` et `StudyCommunities`; la logique métier des semaines, parcours, progressions, marques, lecteur et Cercle est conservée.
