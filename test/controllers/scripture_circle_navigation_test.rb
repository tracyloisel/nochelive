...
class ScriptureCircleNavigationTest < ActionDispatch::IntegrationTest
  ...
  test "makes the ward circle reachable from La Parole and the global menu" do
    sign_in_person(@person)

    get study_program_path

    assert_response :success
    assert_select ".study-path-nav a[href=?]", scripture_circle_path,
      text: I18n.t("scripture_circle.hub_link")
    assert_select ".chrome-drawer a.home-menu-row[href=?]", scripture_circle_path,
      text: /#{Regexp.escape(I18n.t("scripture_circle.hub_link"))}/
  end
  ...
  test "does not advertise a Circle that the player's ward has closed" do
    ...
    assert_response :success
    assert_select ".study-path-nav a[href=?]", scripture_circle_path, count: 0
    assert_select ".chrome-drawer a.home-menu-row[href=?]", scripture_circle_path, count: 0
  end
