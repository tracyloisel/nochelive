...
class Nights::StartTest < ActiveSupport::TestCase
  test "creates a scheduled Noche and snapshots every ward team" do
    ...
    ward = wards(:demo)
    night = Nights::Start.call(ward:, starts_at:, quiz_ids: %w[coronas moises], duration_hours: 3)

    assert_equal starts_at, night.starts_at
    assert_equal starts_at + 3.hours, night.ends_at
  ...
  test "rejects an unsupported duration" do
    assert_raises(ArgumentError) do
      Nights::Start.call(ward: wards(:blank), starts_at: 1.day.from_now, quiz_ids: %w[coronas], duration_hours: 9)
    end
  end
end
